...
int main(int argc, char* argv[]) {
    ...
    int m; long int n;
    memset(nOverlap, 0, sizeof(nOverlap[0]) * maxDF);
	ReadParam(argv[1]);
    for (m = 0; m < 23; m++) {
        for (n = 0; n < 10000; n++) {
...
////
	...
	for (k = 0; k < nInfile; k++) {
        ...
        if (InFile == NULL) {
            ...
            BaseScoreCase[k] = BaseScoreControl[k] = 0; 
    ...
    if (Zthres > 0.0) {
        GetCorrR(nInfile);
        for (k = 0; k < nInfile; k++)
            nOverlap[k] = sqrt(nCase[k]*nCaBase)*CorrR[k] + sqrt(nControl[k]*nConBase)*CorrR[k];
    }
    for (k = 0; k < nInfile; k++) {
        q[k] = nOverlap[k]/(double)(nCase[k]+nControl[k]);
        if (Zthres > 0.0) 
