        ...
        for (n = 0; n < 10000; n++) {
            for (i = 0; i < hashLen[m][n]; i++) {
                ...
                for (j = 0; j < size; j++) {
                    ...
                    if ((FreqMat[j].pPop < epsilon) && (SNP.nCase[j]+SNP.nControl[j] > 0)) {
                        SNP.totalNCase -= SNP.nCase[j];
                        SNP.totalNControl -= SNP.nControl[j];
                        fprintf(LogFile, "Please double check the sample size for SNP %s in the %ld-th input study\n", SNP.SNP, j+1);
                    }
                }
                GenoEnCode[0] = 0; GenoEnCode[1] = 1; GenoEnCode[2] = 2;
                ...
                if (monoFlag == 0) {
                    iter = 0;
                    SNPout = CountLogit(Beta0, GenoMatOut, EnCode, size, FirthFlag, &iter);
                    if (iter == iterMax) {
                        fprintf(LogFile, "Logistic regression for SNP %s did not converge\n", SNP.SNP);
                    }
                }
                else {
